...
| [adr/](adr/) | Architecture decision records — decisions made since this index was written ([ADR-0001](adr/0001-distribution-surfaces.md): distribution surfaces; [ADR-0002](adr/0002-report-binding-mismatch-never-repair-it.md): report a binding mismatch, never repair it; [ADR-0003](adr/0003-resident-endpoint.md): the endpoint is a resident process; [ADR-0004](adr/0004-ground-truth-the-open-profile.md): ground-truth the open profile against the running app; [ADR-0005](adr/0005-layered-rearchitecture-local-landing-writes.md): the layered rearchitecture with local-landing writes, whose [target-architecture spec](adr/0005-target-architecture-spec.md) is the contract the current code implements) |
