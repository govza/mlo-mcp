...
export const cloudDeleteTaskTool = defineTool({
  name: "cloud_delete_task",
  title: "Delete tasks through local cloud sync",
  description:
    ...
    "subtrees to have a recoverable GUID; otherwise nothing is queued — use delete_task instead.",
  inputSchema: {
  ...
  async execute({ ids }, ctx) {
    // Unlike cloud_add_task, the pre-sync export is mandatory: it is the only
    ...
    if (missingGuid.length > 0) {
      ...
      throw new Error(
        `no recoverable GUID for ${list} — nothing was queued (a partial subtree tombstone could orphan children); ` +
          "use delete_task for this batch"
      );
    }
