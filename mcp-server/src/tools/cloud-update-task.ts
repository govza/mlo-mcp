...
const CloudUpdateEntry = z.object({
  ...
  CompletionDateTime: z.string().optional().describe('"" reopens (or use cloud_uncomplete_task)'),
...
export function updatePatch(spec: CloudUpdateSpec, known: KnownRow, now: string, move?: MoveDestination): Record<string, string> {
  ...
  if (spec.DueDateTime !== undefined || spec.StartDateTime !== undefined) {
    ...
    // Mirror cloud_add_task: dates need ScheduleType 1; both cleared → back to 0.
...
export const cloudUpdateTaskTool = defineTool({
  name: "cloud_update_task",
  title: "Update tasks through local cloud sync",
  description:
    ...
    "Only works for tasks whose complete record is in the delta log (added via a cloud tool or changed in MLO " +
    "since the local endpoint took over); date edits on recurring tasks are refused. Booleans (IsProject, Starred, " +
    "Hide*), Flag, Places, and dependsOn are NOT supported here — use update_task for those or any task without a " +
    "logged record.",
  inputSchema: {
    ...
    return runCloudRowUpdate(ctx, [...specs.keys()], {
      ...
      prepare(before: TaskNode[], targets: CloudRowTarget[]) {
        for (const { id, task } of targets) {
          ...
          if (!dest.Guid) {
            throw new Error(`no recoverable GUID for move destination [${moveTo}] "${dest.Caption}" — use update_task`);
          }
