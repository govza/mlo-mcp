...
import { FakeRowStoreView } from "../fakes/fake-row-store.js";
...
describe("IdentityService", () => {
  ...
  it("builds exactly one resolver per snapshot", () => {
    const identity = new IdentityService(new FakeRowStoreView());
  ...
  it("resolves confirmed when the row store holds the UID", () => {
    const rows = new FakeRowStoreView();
    rows.set(UID_A, "annotated");
    const resolution = new IdentityService(rows).resolverFor(snap).uidFor("1");
  ...
  it("resolves unconfirmed on a row-store gap — never a guess", () => {
    const resolution = new IdentityService(new FakeRowStoreView()).resolverFor(snap).uidFor("1");
  ...
  it("refuses with a typed reason for an unknown id and for a task without a recovered GUID", () => {
    const resolver = new IdentityService(new FakeRowStoreView()).resolverFor(snap);
