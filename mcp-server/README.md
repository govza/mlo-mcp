...
Full reference documentation lives in [`../docs/`](../docs/README.md): the mlo.exe CLI, the XML and `.ml` binary formats, and the server architecture. An example Claude Code skill that layers GTD conventions on top of these tools (the server itself is methodology-neutral) is in [`../skills/mlo-gtd/`](../skills/README.md).
