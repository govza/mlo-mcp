...
- **Auto-initialization** — the resident binding a profile by itself after a proxied sync, behind three guards (no existing binding, exactly one candidate UID, the candidate ground-truths against the app's open file) and with the binding written last, so any partial failure leaves none behind. There is no bootstrap tool to call.
...
- **QuickSync** — `mlo.exe -QuickSync`; a best-effort accelerator, never load-bearing, and it opens no session at all when MLO believes nothing changed. What delivers a write is MLO's own ~90 s sync cadence, nudged by the endpoint ([ADR-0005](docs/adr/0005-target-architecture-spec.md) §2).
