...
3. **Point MLO's cloud sync through the local endpoint:** in the proxy settings use hostname `http://127.0.0.1` and port `8181`, and leave **"Use secure connection" unchecked**. MLO 6.1.3 rejects the bare hostname as an invalid URL before contacting the proxy. This matters: with secure connection checked MLO still syncs fine, but the endpoint only sees an opaque TLS tunnel, so setup and every write silently stay dead. MLO's sync log prints *"secure connection is OFF for this sync profile"* when it is set correctly. Only the MLO↔endpoint hop on your own machine is plaintext: the endpoint re-encrypts before it leaves your machine, so the leg from the endpoint to MLO's cloud is TLS either way.
...
- `update_task` can set or clear a one-off `ReminderDateTime`, `Bold`, and yellow `Highlight`. It deliberately does not author recurrence, custom alert actions, or other colours/fonts.
- Keep MLO's built-in Inbox caption as `<Inbox>`. The cloud sync identity uses that canonical caption; renaming it prevents safe targeting until it is restored.
