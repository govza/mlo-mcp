...
- **Inbox** = the top-level task captioned `Inbox`.
- Buffer captured items and write them in `add_task` batches (≤25 per call, all
  under the Inbox) — batching avoids restarting the MLO app per item, but don't
  hold everything to the very end of a long session; flush between categories.
